...
enum MessageType: Codable {
    ...
    case echoOkMessage(EchoOkMessage)
    ...
    var type: String {
        ...
        case .echoMessage: return "echo"
        case .echoOkMessage: return "echo_ok"
    ...
    init(from decoder: Decoder) throws {
        ...
        case "echo":
            ...
            self = .echoMessage(decodedMessage)
        case "echo_ok":
            let decodedMessage = try jsonDecoder.decode(
                EchoOkMessage.self,
                from: jsonData
            )
            self = .echoOkMessage(decodedMessage)
        case "error":
            let decodedMessage = try jsonDecoder.decode(
                ErrorMessage.self,
                from: jsonData
    ...
    func encode(to encoder: Encoder) throws {
        ...
        case .initOkMessage(let message):
            try container.encode(message)
        case .echoMessage(let message):
            try container.encode(message)
        case .echoOkMessage(let message):
            try container.encode(message)
